...
#define _card_enable()      	HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4, GPIO_PIN_RESET); //NRF_GPIO->OUTCLR = (1UL << SD_CS) 

#define _card_disable()     	HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4, GPIO_PIN_SET); //NRF_GPIO->OUTSET = (1UL << SD_CS)
#define _card_power_on()
#define _card_insert()       	0 //GPIO_ReadInputDataBit(GPIOB,GPIO_Pin_0)


#define SPI1_CS_OUTPUT() do { NRF_GPIO->DIRSET = (1UL << SD_CS); } while(0)
#define SPI1_SCK_OUTPUT() do { NRF_GPIO->DIRSET = (1UL << SD_CLK); } while(0)
#define SPI1_MOSI_OUTPUT() do { NRF_GPIO->DIRSET = (1UL << SD_MOSI); } while(0)
#define SPI1_MISO_INPUT() do { NRF_GPIO->DIRCLR = (1UL << SD_MISO); } while(0)
#define SPI1_CS_HIGH()   do { HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4, GPIO_PIN_SET);} while(0)   //NRF_GPIO->OUTSET = (1UL << SD_CS);   
#define SPI1_CS_LOW()    do { HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4, GPIO_PIN_RESET);} while(0) //NRF_GPIO->OUTCLR = (1UL << SD_CS);  
...
__inline uint16_t _spi_read_write(uint8_t data);
